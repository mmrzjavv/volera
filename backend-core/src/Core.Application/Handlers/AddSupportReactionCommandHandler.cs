...
public class AddSupportReactionCommandHandler : IRequestHandler<AddSupportReactionCommand>
{
    ...
    public AddSupportReactionCommandHandler(
    ...
    public async Task Handle(AddSupportReactionCommand request, CancellationToken cancellationToken)
    {
        ...
        else
        {
            if (existing == null)
            {
                var reaction = new Core.Domain.Entities.MessageReaction(request.MessageId, request.SupportUserId, request.Emoji, fromSupport: true);
